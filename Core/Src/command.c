...
#define DOWNLOAD_FILENAME "Thulium_laser.bin"
#define FILENAME_TO_FIND "Thulium_laser.bin"
// Program Size: Code=27924 RO-data=1856 RW-data=108 ZI-data=38036
// Program Size: Code=18100 RO-data=472 RW-data=80 ZI-data=4064
// Program Size: Code=18100 RO-data=472 RW-data=80 ZI-data=4064  
/* Private macros ------------------------------------------------------------
 */
/* Private variables ---------------------------------------------------------
 */
static uint32_t TmpReadSize;
static uint32_t RamAddress;
 ...
 */
static void program_flash_memory(void);
...
void COMMAND_Download(void)
{
    ...
    if (strcmp(file_name, FILENAME_TO_FIND) == 0) {
        printf("target .bin found: %s\n", file_name);
    } else {
        printf("no target bin\n");
        return;
    }
...
static void program_flash_memory(void)
{
    ...
    printf("total %lu bytes programmed\n", total_size);


    /* verify programmed bin */
    printf("verifying...");
    f_lseek(&down_load_file, 0);
    readflag = TRUE;
    LastPGAddress = APPLICATION_ADDRESS;
    ...
    while (readflag) {
        ...
        /* check flash memory */
        for (uint32_t i = 0; i < bytesread; i++, LastPGAddress++) {
            if (RAM_Buf[i] != *(uint8_t *)LastPGAddress) {
                /* Flash programming error: Turn LED3 On and Toggle LED4 in infinite
                * loop */
                // BSP_LED_On(LED3);
...
FRESULT find_file(const TCHAR *path, const TCHAR *ext, TCHAR *foundFile)
{
    ...
    while (1) {
        ...
        // if (fileInfo.fattrib & AM_ARC && strcmp(fileInfo.fname +
        // strlen(fileInfo.fname) - 3, ext) == 0)
